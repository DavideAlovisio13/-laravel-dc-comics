            ...
            <form action="{{ route('comics.update', $comic->id) }}" method="POST">
                ...
                <div class="mb-3 d-flex justify-content-center align-items-center gap-2">
                    ...
                    <input type="text" class="form-control" id="title" aria-describedby="titleHelp" name="title"
                        value="{{ old('title', $comic->title) }}" required maxlength="250" minlength="5">
                    @error('title')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <span class="check px-2"></span>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3 d-flex justify-content-center align-items-center gap-2">
                    ...
                    <input type="text" class="form-control" id="series" name="series"
                        value="{{ old('series', $comic->series) }}" required maxlength="250" minlength="5">
                    @error('series')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <span class="check px-2"></span>
